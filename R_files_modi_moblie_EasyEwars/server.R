...
library(tidyr)
library(lubridate)
# library(pse)# for automatic calibration ## อันนี้ Package 'pse' was removed from the CRAN repository.
library(reportROC)
library(caret)
library(e1071)
library(knitr)
...
server<-function(input,output,session) { 
  ...
  # observeEvent(input$getdata_ggdrive,{
  #   
  #   print(paste("getdata ggdrive status:", input$getdata_ggdrive))
  #   
  #   if (input$getdata_ggdrive == "TRUE"){
  #     
  #     output$dataTable  <- renderUI({
  #       f7Table(mydata)
  #     })
  #     
  #     # getData<-readLines("GetDataFromGoogledrive_func.R",warn=FALSE)
  #   
  #     
  #   }else if (input$getdata_ggdrive == "FALSE"){
  #     mydata <- NULL
  #     
  #     output$dataTable  <- renderUI({
  #       f7Table(mydata)
  ...
  # output$dataTable  <- renderUI({
  #   f7Table(values$mydata)
  # })
  
  
  #----------- messageCheckData ----------
  values <- reactiveValues(messageCheckDataText_1="",
                           messageCheckDataText_2="",
                           messageCheckDataText_3="",
                           messageCheckDataText_4="",
                           messageCheckDataText_5="",
                           messageCheckDataText_6=""
                           )
  
  
  output$messageCheckData_1<-renderText(
    paste(values$messageCheckDataText_1)
  )
  
  output$messageCheckData_2<-renderText(
    paste(values$messageCheckDataText_2)
  )
  
  output$messageCheckData_3<-renderText(
    paste(values$messageCheckDataText_3)
  )
  
  output$messageCheckData_4<-renderText(
    paste(values$messageCheckDataText_4)
  )
  
  output$messageCheckData_5<-renderText(
    paste(values$messageCheckDataText_5)
  )
  
  output$messageCheckData_6<-renderText(
    paste(values$messageCheckDataText_6)
  )
  
  
  # อันนี้ต้องทำสองแบบ คือ observeEvent ของ session และ observeEvent ของ input$dat
  
  # 1: observeEvent ของ session
  # observeEvent(session, {
  #   # print("input$dat: ")
  #   # print(input$dat) # NULL
  #   
  #   
  #   if (is.null(input$dat)){
  #     values$messageCheckDataText_1 <- "📌 Error: There are no data have been uploaded on the Dataset tab ❗"
  #     values$messageCheckDataText_2 <- "📌 Error: There are no data have been uploaded on the Dataset tab ❗"
  #     values$messageCheckDataText_3 <- "📌 Error: There are no data have been uploaded on the Dataset tab ❗"
  #     values$messageCheckDataText_4 <- "📌 Error: There are no data have been uploaded on the Dataset tab ❗"
  #     values$messageCheckDataText_5 <- "📌 Error: There are no data have been uploaded on the Dataset tab ❗"
  #     values$messageCheckDataText_6 <- "📌 Error: There are no data have been uploaded on the Dataset tab ❗"
  #     
  #     return(NULL)
  #     
  #   }
  #   
  #   else if (!is.null(input$dat)){
  #     values$messageCheckDataText_1<-NULL
  #     values$messageCheckDataText_2<-NULL
  #     values$messageCheckDataText_3<-NULL
  #     values$messageCheckDataText_4<-NULL
  #     values$messageCheckDataText_5<-NULL
  #     values$messageCheckDataText_6<-NULL
  #     
  #     
  #     return(NULL)
  #     
  ...
  # เหมือนจะใช้ไม่ได้ มันไม่ยอมหาย
  # observeEvent(input$dat, {
  #   showF7Preloader(target = "#plot1", color = "blue")
  #   Sys.sleep(2)
  #   f7HidePreloader()
  # })
